        ...
        with self.browser() as ctx:
            ...
            for date in dates:
                url = self.URL_TPL.format(
                    ...
                    from_name=urllib.parse.quote(
                        self.city_name(from_city, getattr(self, "_route_from_name", None))),
                    to_name=urllib.parse.quote(
                        self.city_name(to_city, getattr(self, "_route_to_name", None))),
                    date=date,
                )
                ...
                try:
                    ...
                    if getattr(self, "_route_flight_nos", None):
                        self.logger.warning("[ctrip] 暂不支持按航班号筛选，已跳过")
                        price = None
